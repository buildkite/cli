...
func (m model) headerView(width int) string {
	sections := []string{
		...
		mutedStyle.Render(wrapText(
			"Normal screen mode. External messages update independent sections. Resize the terminal to watch wrapped lines reflow. Use the viewport to scroll the live build output below. Press q to quit.",
			width,
		)),
		"",
	...
	sections = append(sections,
		...
		"",
		headingStyle.Render("Live Build Output"),
	)
...
func (m model) footerView() string {
	if m.viewport.TotalLineCount() == 0 {
		return mutedStyle.Render("Scroll: ↑/↓ PgUp/PgDn • Quit: q")
	}

	return mutedStyle.Render(fmt.Sprintf(
		"Scroll: ↑/↓ PgUp/PgDn • %d lines • %.0f%% through output • Quit: q",
...
func (m *model) syncViewport() {
	width := m.contentWidth()
	headerHeight := lipgloss.Height(m.headerView(width))
	...
	stickToBottom := m.viewport.TotalLineCount() == 0 || m.viewport.AtBottom()
	m.viewport.Width = width
	m.viewport.Height = viewportHeight
	m.viewport.SetContent(m.viewportContent(width))
	if stickToBottom || m.viewport.PastBottom() {
