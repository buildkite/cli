...
// FormatFailedTestsLive renders the full failed tests section as lines for the live region.
// Tests are capped at MaxDisplayedTests with an overflow hint.
func FormatFailedTestsLive(tests []FailedTest, buildNumber int) []string {
	if len(tests) == 0 {
		return nil
	}
	...
	var lines []string
	lines = append(lines, "")
	lines = append(lines, fmt.Sprintf("  ── Failed Tests (%d) %s", len(tests), strings.Repeat("─", 60)))
	lines = append(lines, "")

	display := tests
	if len(tests) > MaxDisplayedTests {
		display = tests[:MaxDisplayedTests]
	}
	...
	if overflow := len(tests) - MaxDisplayedTests; overflow > 0 {
		lines = append(lines, fmt.Sprintf("  … and %d more — run `bk build test failures %d` to view all", overflow, buildNumber))
	}
