...
// FormatFailedTests renders a failed tests section using the standard table formatter.
// Returns empty string if there are no failed tests.
func FormatFailedTests(tests []FailedTest, buildNumber int) string {
	if len(tests) == 0 {
		return ""
	}
	...
	if truncated > 0 {
		sb.WriteString(fmt.Sprintf("\n  … and %d more — run `bk build test failures %d` to view all\n", truncated, buildNumber))
	}
